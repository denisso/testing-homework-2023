...
Установка значений BUG_ID выполняется с помощью cross-env в файле package.json, прогоны тестов запускать npm run test. На каждый баг падает 1 - 2 теста. тесты выполнялись на v16.14.0.

BUG_ID=2 не обрабатывается, BUG_ID 4 обрабатывается в hermione, остальные в модульных тестах jest

AssertViewError: image comparison failed лечится через hermione gui
